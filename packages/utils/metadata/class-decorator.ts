...
import { extendObjectMetadata, ExtendObjectMetadata, SetMetadata } from './base-decorators';
...
>(key: string, startingValue: TValueType): IClassMergeObjectMetadata<TValueType> {
  return {
    decorator: (value: Partial<TValueType> = {}) => ExtendObjectMetadata<TValueType>(key, value, startingValue),
    getter: (target) => Reflect.getMetadata(key, target) || null,
  };
}
