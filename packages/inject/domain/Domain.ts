...
export class Domain {
  ...
  constructor(
    ...
    private readonly _children: Array<Domain>,
    ...
    this.logger = logger.child({ component: this.constructor.name, domainName: name });
    this.logger.trace({ parentName: parent?.name }, 'Constructing domain.');
  ...
  get children(): ReadonlyArray<Domain> { return this._children; }
  ...
  static fromDefinition(
    def: IDomainDefinition,
    parent: Domain | null = null,
    ...
    const children = (def.children || []).map(c => Domain.fromDefinition(c, d, nameStack));
